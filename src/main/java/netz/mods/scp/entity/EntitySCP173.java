...
public class EntitySCP173 extends SCPEntity
{
	public Minecraft mc = Minecraft.getMinecraft();
	public EntityPlayerHelper eph;
	...
	public EntitySCP173(World world)
	{
		super(world);
	...
	protected void applyEntityAttributes()
	{
		...
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setAttribute(16.0D);
		this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setAttribute(200.0D);
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setAttribute(9001.0D);
	}
	
	protected void playStepSound(int par1, int par2, int par3, int par4)
	{
		this.playSound("mob.173.step", 0.15F, 1.0F);
	}
...
//				}
	...
	{
		List<?> var5 = this.worldObj.getEntitiesWithinAABB(EntitySCP131.class, this.boundingBox.expand((double)4F, 2.0D, (double)4F));
		...
		if (mc.thePlayer != null && eph.getBlink() >= 0 && eph.getBlink() <= 10) return false;
	...
	{
		return rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)getEyeHeight(), posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)height, posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)height * 0.10000000000000001D, posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX + 0.69999999999999996D, posY + (double)getEyeHeight(), posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX - 0.69999999999999996D, posY + (double)getEyeHeight(), posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)getEyeHeight(), posZ + 0.69999999999999996D), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)getEyeHeight(), posZ - 0.69999999999999996D), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)height * 1.2D, posZ - 0.69999999999999996D), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null || rayTraceBlocks(Vec3.createVectorHelper(posX, posY + (double)height * 1.2D + 1.0D, posZ), Vec3.createVectorHelper(entityPlayer.posX, entityPlayer.posY + (double)entityPlayer.getEyeHeight(), entityPlayer.posZ)) == null;
	}
	...
	public boolean SCPSeeSCP(EntitySCP173 entityscp)
	{
		if (worldObj.getFullBlockLightValue(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ)) < 1)
		{
			return false;
	...
	private boolean isInFieldOfVision(EntitySCP173 entityscp, EntityPlayer entityPlayer, float f, float f1)
	{
		float f2 = entityPlayer.rotationYaw;
		float f3 = entityPlayer.rotationPitch;
		entityPlayer.faceEntity(entityscp, 360F, 360F);
		...
		float f6 = f;
		float f7 = f1;
		float f8 = entityPlayer.rotationYaw - f6;
		float f9 = entityPlayer.rotationYaw + f6;
		float f10 = entityPlayer.rotationPitch - f7;
		float f11 = entityPlayer.rotationPitch + f7;
		boolean flag = GetFlag(f8, f9, f2, 0.0F, 360F);
		boolean flag1 = GetFlag(f10, f11, f3, -180F, 180F);
		return flag && flag1 && (entityPlayer.canEntityBeSeen(entityscp) || LineOfSightCheck(entityPlayer));
	}
